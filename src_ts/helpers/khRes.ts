...
export default class ResHelper {
  ...
  /**
     ...
     */
  async checkAsset (account_name: string) : Promise<Asset> {
  ...
  async issue (account_name: string, quantity: string, memo: string) {
    let contract = await this.resContract()
    return await contract.resissue({ user : account_name, quantity, memo })
  }

  async burn (account_name: string, quantity: string, memo: string) {
    let contract = await this.resContract()
    return await contract.resburn({ user : account_name, quantity, memo })
  }

  async take (from: string, to: string, quantity: string, memo: string) {
    let contract = await this.resContract()
    return await contract.restake({ from, to, quantity, memo })
  }

  async change (account_name: string, from: string, to: string, memo: string) {
    let contract = await this.resContract()
